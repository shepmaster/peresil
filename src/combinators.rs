...
use super::{ParseMaster, Progress, Status, Point, Recoverable};
          ...
          //E: Recoverable, // TODO: use this
{
    move |pm, pt| {
        ...
        match parser(pm, pt) {
            Progress { point, status: Status::Success(v) } => {
                append_to.push(v);
                Progress::success(point, append_to)
            }
            Progress { point, status: Status::Failure(..) } => {
                Progress::success(point, append_to)
            }
        }
    }
