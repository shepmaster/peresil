...
macro_rules! try_parse(
    ($e:expr) => ({
        match $e {
            $crate::Result::Success(progress) =>
                progress.to_tuple(),
            $crate::Result::Partial{ failure: pf, .. } |
            $crate::Result::Failure(pf) =>
                return $crate::Result::Failure(pf),
        }
    });
    ($e:expr, $err:expr) => ({
        match $e {
            ...
            $crate::Result::Partial{ failure: pf, .. } |
            $crate::Result::Failure(pf) => {
                let fail = $crate::Progress{ data: Some($err), ..pf };
                return $crate::Result::Failure(fail);
            },
        }
    });
);
...
macro_rules! try_partial_parse(
    ($e:expr) => ({
        match $e {
            $crate::Result::Success(progress) =>
                (progress.data, $crate::BestFailure::new(), progress.point),
            $crate::Result::Partial{ success: s, failure: pf } =>
                (s.data, $crate::BestFailure::with(pf), s.point),
            $crate::Result::Failure(pf) =>
                return $crate::Result::Failure(pf),
        }
    })
    ...
    ($partial:expr, $e:expr) => ({
        match $e {
            ...
            $crate::Result::Failure(pf) => {
                ...
                partial.push(pf);
                return $crate::Result::Failure(partial.pop())
            },
        }
    });
);
