...
fn parse_unit<'a>(
    ...
    pt.consume_identifier(identifiers)
        .map_err(|_| Error::UnknownUnit)
}
...
fn failure_expected_whitespace_after_size() {
    assert_eq!(parse("5cup"), Err((1, vec![Error::ExpectedWhitespace])));
}

#[test]
fn failure_unknown_unit() {
    assert_eq!(parse("100 grains rice"), Err((4, vec![Error::UnknownUnit])));
}

#[test]
fn failure_expected_whitespace_after_unit() {
    assert_eq!(parse("10 cups"), Err((7, vec![Error::ExpectedWhitespace])));
}

#[test]
fn failure_expected_name() {
    assert_eq!(parse("10 cups "), Err((8, vec![Error::ExpectedName])));
}
